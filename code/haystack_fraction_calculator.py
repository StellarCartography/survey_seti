# based on code: https://github.com/shbhuk/SETI_haystack
# published: https://doi.org/10.3847/1538-3881/aae099
...
def vol8d(home='', verbose=False, hayfile='Haystack_boundaries.txt', searchfile='Search_parameters.txt'):
    '''
    ...
    # allow users to specify their own "home directory" to read from
    if len(home)<1:
        home = sys.path[0]

    # allow users to specify their own input files
    haystack_boundary = os.path.join(home, hayfile)
    search_parameters = os.path.join(home, searchfile)
    ...
    # Total Sky Coverage [sq. degrees]. If > 0, then the number of observations,
    # and the telescope diameter are ignored.
    ...
    mean_band_frequency = (band_max + band_min)/2 # Mean frequency of instrument band.
    # e.g. for GBT L Band it is about ~1.5 GHz
    ...
    # return the haystack volume, and volume scanned
    return V_total_8d, V_scanned_8d
