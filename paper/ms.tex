...
Rather than searching for an exoplanet beacon consisting of outliers in spatial density or distribution (e.g. \S\ref{sec:2pt}), we can consider clusters of systems that are unusually similar in some way. In the simplest case this could take the form of a group of closely clustered stars with transiting exoplanets with the same radius, transit depth, transit ephemeris (or transit midpoint), or orbital period, for example. The assumption here is that a civilization would have engineered this unusual set of transiting systems in order to be noticed by early astronomers (us) hoping to characterize the exoplanet demographics of our Galaxy.
...
make clusters using easy KNN algorithm
